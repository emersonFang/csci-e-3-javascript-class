...
(function(){
     ...
     * */
    window.onload = function dataExists() { //this doesn't work.  wrong scope, fixed it by pulling out tihs function and writeRowToPage
    var localPeople = window.localStorage.getItem("people");
    if (localPeople) { 
        var localPersonArray = JSON.parse(localPeople);

        for (var i = 0; i<localPersonArray.length; i++) {
            writeRowToPage(localPersonArray[i],output);
        }
        }
    }
	...
	personArray.push(person);	
        window.localStorage.setItem("people",JSON.stringify(personArray));
	} else {
	localPersonArray.push(person);
	window.localStorage.setItem("people",JSON.stringify(localPersonArray));	
    }
